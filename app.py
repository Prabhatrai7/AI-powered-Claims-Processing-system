...
if api_key is None or api_key == "":
    print("Api key is not defined, please update API key correctly and try again..")
    exit()
...
def get_claim_approval_content():
    db = get_embedddings()
    context = db.similarity_search("What are documents required for claim approval?")
...
def get_general_exculsion_context():
    db = get_embedddings()
    context = db.similarity_search("Give list of all general exclusions")
...
def get_bill_info(data):
    ...
    client = ChatOpenAI(model="mistralai/devstral-2512:free", base_url="https://openrouter.ai/api/v1", api_key=api_key, temperature=0.2, max_tokens=1500)
...
def check_claim_rejection(claim_reason, general_exclusion_list, prompt_template, threshold=0.2):
...
def msg():
    ...
    if bill_info['expense'] != None and int(bill_info['expense']) < int(total_claim_amount):
        claim_validation_message = "The amount mentioned for claiming is more than billed amount. Claim Rejected!"
        ...
        llm = ChatOpenAI(model="mistralai/devstral-2512:free", base_url="https://openrouter.ai/api/v1")
        llmchain = LLMChain(llm=llm, prompt=prompt_template)
        
        output = llmchain.run({"claim_approval_context": get_claim_approval_content(), "general_exclusion_context": general_exclusion_list, "patient_info": patient_info, "medical_bill_info": medical_bill_info, "max_amount": total_claim_amount, "disease": bill_info['disease']})
        output = re.sub(r"\n", "<br>", output)
        ...
        output = "Please add a valid consulation receipt"
        return render_template("result.html", name=name, address=address, claim_type=claim_type, claim_reason=claim_reason, date=date, medical_facility=medical_facility, total_claim_amount=total_claim_amount, description=description, output=claim_validation_message)
